...
var list = [
  ...
  function renderProduct(theloaichon_arr=[]) {
    var product =document.getElementById('product')
      product.innerHTML='';
      for(i=0; i<list.length;i++){
        theloaiTruyen=list[i].theloai;
        image=list[i].image;
        ten=list[i].title;
        link=list[i].Link
      if(theloaichon_arr.length>0){
        if(theloaichon_arr.includes(list[i].theloai)==false)continue;
      }
